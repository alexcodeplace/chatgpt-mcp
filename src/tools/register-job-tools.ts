...
export function registerJobTools(server: Pick<McpServer, 'registerTool'>, config: Readonly<ChatGptMcpConfig>, concurrency: ConcurrencyController): void {
  ...
  server.registerTool('exec.start', {
    ...
    description: config.keyManager.enabled
      ? 'Preferred for builds and other long commands. For operations needing a named API key or service credential, use kmgr.list, kmgr.profiles, kmgr.run and kmgr.status instead of reading a credential file; the key value stays inside the broker. Supply a unique operationId ONCE, retain the returned jobId, then use exec.status/output. Repeating the SAME operationId and arguments retrieves the existing job and never replays it within retention. An unknown outcome must be reconciled, not retried with a new ID. Survives tunnel reconnects; systemd workers also survive backend restarts. No implicit shell.'
      : 'Preferred for builds and other long commands. Completed stdout and stderr are stored and returned unchanged, subject to configured output and retention limits. Supply a unique operationId ONCE, retain the returned jobId, then use exec.status/output. Repeating the SAME operationId and arguments retrieves the existing job and never replays it within retention. An unknown outcome must be reconciled, not retried with a new ID. Survives tunnel reconnects; systemd workers also survive backend restarts. No implicit shell.',
