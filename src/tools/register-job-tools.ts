import type { ToolRegistrar } from './redacted-tools.js';
...
export function registerJobTools(server: ToolRegistrar, config: Readonly<ChatGptMcpConfig>, concurrency: ConcurrencyController): void {
  ...
  server.registerTool('exec.start', {
    ...
    description: 'Preferred for builds and other long commands. Authorized local credentials may be consumed internally; returned logs redact secret text to [SECRET_REDACTED], not block execution. Supply a unique operationId ONCE, retain the returned jobId, then use exec.status/output. Repeating the SAME operationId and arguments retrieves the existing job and never replays it within retention. An unknown outcome must be reconciled, not retried with a new ID. Survives tunnel reconnects; systemd workers also survive backend restarts. No implicit shell.',
  ...
  }, async ({ operationId, command, args, cwd, env, timeoutMs }, ctx) => invoke('exec.start', ctx.mcpReq.signal, () => store.start(operationId, {
  ...
  }, async ({ jobId }, ctx) => invoke('exec.status', ctx.mcpReq.signal, () => store.status(jobId)));
  server.registerTool('exec.output', {
    title: 'Durable Command Output', description: 'Retrieve bounded stdout or stderr after command completion. Secret text is redacted before persistence and pagination. Use nextOffset to read the next UTF-16 character slice of sanitized output. Output survives tunnel reconnects until output retention expires.',
  ...
  }, async ({ jobId, stream, offset, maxCharacters }, ctx) => invoke('exec.output', ctx.mcpReq.signal, () => store.output(jobId, stream, offset, maxCharacters)));
  ...
  }, async ({ jobId }, ctx) => invoke('exec.cancel', ctx.mcpReq.signal, () => store.cancel(jobId)));
  server.registerTool('exec.list', {
    ...
    annotations: { readOnlyHint: true, idempotentHint: true, openWorldHint: false },
  }, async ({ limit }, ctx) => invoke('exec.list', ctx.mcpReq.signal, async () => ({ jobs: await store.list(limit) })));
}
