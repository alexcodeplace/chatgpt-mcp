...
export function registerJobTools(server: McpServer, config: Readonly<ChatGptMcpConfig>, concurrency: ConcurrencyController): void {
  ...
  server.registerTool('exec.start', {
    ...
    description: 'Preferred for builds and other long commands. Supply a unique operationId ONCE, retain the returned jobId, then use exec.status/output. Repeating the SAME operationId and arguments retrieves the existing job and never replays it within retention. An unknown outcome must be reconciled, not retried with a new ID. Survives tunnel reconnects; systemd workers also survive backend restarts. No implicit shell.',
  ...
  server.registerTool('exec.output', {
    title: 'Durable Command Output', description: 'Retrieve bounded stdout or stderr after command completion. Use nextOffset to read the next UTF-16 character slice. Output survives tunnel reconnects until output retention expires.',
