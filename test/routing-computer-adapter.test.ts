...
test('explicitly long local calls still execute through the local adapter', async () => {
  const cwd = await root();
  try {
    const config = parseConfig({
      filesystem: { roots: [cwd] },
      shell: { enabled: true, allowedCommands: ['node'], maxRuntimeMs: 2_000, defaultRuntimeMs: 50, maxOutputBytes: 4096 },
      execution: { kubernetes: { enabled: false, client: { command: 'this-client-must-never-be-invoked' } } },
    });
    const adapter = new RoutingComputerAdapter(config);
    const request = { command: 'node', args: ['-e', 'process.stdout.write("long-local")'], cwd, timeoutMs: 500 } as const;
    assert.equal(adapter.classifyExec(request), 'shell-local-long');
    const result = await adapter.exec(request);
    assert.equal(result.exitCode, 0);
    assert.equal(result.stdout, 'long-local');
    const metrics = adapter.executionMetrics() as { routing: { local: number; remote: number } };
    assert.equal(metrics.routing.local, 1);
    assert.equal(metrics.routing.remote, 0);
  } finally {
