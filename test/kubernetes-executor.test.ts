...
if (command[0] === 'sh' && command[1] === '-c') { process.stdout.write('/usr/bin/fake\\n'); process.exit(0); }
  ...
  return parseConfig({
    ...
    execution: {
      kubernetes: {
        ...
        workspace: { containerPath: '/configured-workspace', maxArchiveBytes: 1024 * 1024 },
...
test('generic Kubernetes executor uses only configured cluster/image/policy values and cleans its pod', async () => {
  ...
  try {
    ...
    const result = await executor.exec({ command: 'node', args: ['-e', 'ignored'], cwd: fixture.root, env: { TEST: '1' } }, 5_000, 4096);
