...
If the persistent router's exact configured Node executable is replaced by the operating system while the router is still running, normal reconciliation may repair that stale executable identity without replacing backends, ingress, or tunnels. The repair is allowed only when the router argv still selects the recorded router release, the /proc executable target is exactly the configured Node path with the kernel's deleted-file marker, every routed generation has zero in-flight calls, and the persisted routing identity (epoch, active/previous selection, legacy owner and generation descriptors) is unchanged across the router-only restart. Tunnel and ingress main processes are paused and resumed with the same PIDs around that bounded restart. Any other executable or route drift remains a hard provenance failure.
